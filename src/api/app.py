...
#     try:
#         response = requests.get(f"{BACKEND_URL}/api/merged-data")
#         if response.status_code == 200:
#             return pd.DataFrame(response.json())
#         else:
#             st.error("Failed to load data from backend server API.")
#             return pd.DataFrame()
...
# @st.cache_data
# def fetch_content_profiles():
#     try:
#         response = requests.get(f"{BACKEND_URL}/api/content-profiles")
#         if response.status_code == 200:
#             return pd.DataFrame(response.json())
#         else:
#             st.error("Failed to load content profiles from backend server API.")
#             return pd.DataFrame()
#     except Exception as e:
#         st.error(f"Could not connect to backend server: {str(e)}")
#         return pd.DataFrame()
...
# content_df = fetch_content_profiles()

# if df.empty or content_df.empty:
#     st.warning("Waiting for Backend Data Pipeline to become responsive...")
#     st.stop()

# # Ensure types match baseline expectations
...
#     title="Historical Playlist Timeline Explorer", hovermode="x unified"
# )
...
# fig2.update_layout(
#     template="plotly_dark", height=550, title="2027–2028 Predictive Rank Movements",
#     paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(255,255,255,0.02)"
# )
...
# ))
# fig3.update_layout(
#     template="plotly_dark", height=500, title=f"{selected_song} Historical Rank Evolution",
#     paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(255,255,255,0.02)"
# )
...
# ))
# fig4.update_layout(
#     template="plotly_dark", height=550, title=f"{selected_song} Decay / Resurgence Forecast",
#     paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(255,255,255,0.02)"
# )
...
# fig5.update_layout(
#     template="plotly_dark", height=600, title=title,
#     paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(255,255,255,0.02)",
#     xaxis_title="Artist", yaxis_title="Dominance Score"
# )
...
#     fig6.update_layout(
#         template="plotly_dark", height=620, title={"text": f"{scatter_song}: Popularity vs Rank Predictive Matrix", "x":0.03},
#         paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(255,255,255,0.02)",
#         font=dict(family="Arial", size=14), hovermode="closest",
...
#     st.plotly_chart(fig6, use_container_width=True, key=f"advanced_scatter_{scatter_song}")
...
#     fig_album.update_layout(template="plotly_dark", height=500, showlegend=False, paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(255,255,255,0.02)", title="Revenue Performance by Album Packaging Type", font=dict(size=14))
#     fig_album.update_traces(textposition="outside")
#     st.plotly_chart(fig_album, use_container_width=True, key="revenue_vs_album_type")
...
#     fig_packaging.update_layout(template="plotly_dark", height=550, paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(255,255,255,0.02)", title="Packaging Strategy vs Revenue & Track Architecture")
#     st.plotly_chart(fig_packaging, use_container_width=True, key="packaging_strategy_analysis")

#     fig_tracks = px.bar(packaging_df, x="release_packaging_strategy", y="total_tracks", color="release_packaging_strategy", text_auto=".2f")
#     fig_tracks.update_layout(template="plotly_dark", height=450, paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(255,255,255,0.02)", title="Track Architecture Control by Packaging Strategy")
#     st.plotly_chart(fig_tracks, use_container_width=True, key="track_control_analysis")
...
#         fig7.update_layout(template="plotly_dark", height=520, paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(255,255,255,0.02)", title="Explicit vs Clean Behavioral Performance Matrix")
#         st.plotly_chart(fig7, use_container_width=True, key="explicit_clean_intelligence")
...
#         fig_radar.update_layout(template="plotly_dark", height=500, polar=dict(radialaxis=dict(visible=True)), title="Content-Type Behavioral Fingerprint")
#         st.plotly_chart(fig_radar, use_container_width=True, key="behavioral_radar")
...
# =========================================================
# CONFIG & PATH RESOLUTION (Bypassing Backend Server)
# =========================================================
st.set_page_config(
    page_title="Music ML Dashboard",
    layout="wide"
...
# Going up two levels to hit the repository root
...
# Initialize dataframes directly from files
df = load_merged_data()
